...
class group extends Model{	
	...
	public function getAllGroups($userid){
		return $this->sql("SELECT * FROM Users_Groups LEFT JOIN Groups ON (Users_Groups.group_id=Groups.group_id) WHERE permission LIKE ('_1_') AND user_id='{$userid}' ORDER BY group_name ASC;", $return = "array", $key ="group_id");
	...
	{
		return $this->sql("SELECT user_id, name, surname, permission FROM Users_Groups LEFT JOIN User ON (Users_Groups.user_id=User.id_user) WHERE group_id = '{$groupid}';", $return = "array", $key ="user_id");
	}
